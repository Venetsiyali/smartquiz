...
const SUBJECTS = [
    ...
    'Geografiya', "O'zbek tili", 'Ingliz tili', 'Adabiyot', 'Informatika', 'Boshqa',
];
const GRADES = [
...
export default function AdminLibraryPage() {
    ...
    const handleSave = async () => {
        if (!form.title.trim() || form.questions.length === 0) {
            setSaveMsg({ type: 'err', text: "Sarlavha va kamida 1 ta savol kiritilishi shart" });
            return;
        }
        setSaving(true);
        setSaveMsg(null);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-white/30 text-xs font-black tracking-widest uppercase mb-1">
                        Admin · Kutubxona
                    </p>
            ...
            <div className="flex flex-col xl:flex-row gap-6">
                            ...
                            <thead>
                                <tr className="border-b border-white/5">
                                    {['Sarlavha', 'Fan', 'Sinf', 'Savollar', "O'ynalgan", 'Reyting', 'Holat', 'Amallar'].map(h => (
                                        <th key={h} className="text-left px-4 py-3 text-white/25 font-black text-xs uppercase tracking-wider whitespace-nowrap">
                                            {h}
                                        </th>
                                    ))}
                                </tr>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5 whitespace-nowrap">
                                                <button
                                                    ...
                                                    className="px-2.5 py-1 rounded-lg text-xs font-black transition-all hover:scale-105"
                                                ...
                                                <button
                                                    ...
                                                    disabled={actionLoading === quiz.id + '_arch'}
                                                    className="px-2.5 py-1 rounded-lg text-xs font-black transition-all hover:scale-105 disabled:opacity-50"
                                                    style={{ background: 'rgba(251,191,36,0.12)', color: '#fbbf24' }}
                                                    title={quiz.is_active ? 'Arxivlash' : 'Faollashtirish'}
                                                >
                                                    {quiz.is_active ? '📦' : '♻️'}
                                                </button>
                                                <button
                                                    onClick={() => handleDelete(quiz.id)}
                                                    disabled={actionLoading === quiz.id + '_del'}
                                                    className="px-2.5 py-1 rounded-lg text-xs font-black transition-all hover:scale-105 disabled:opacity-50"
                ...
                {/* ── RIGHT: Add / Edit form ──────────────────────────────── */}
                <div
                    className="xl:w-[420px] shrink-0 rounded-2xl p-6 flex flex-col gap-4"
                    style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}
                >
                    <div className="flex items-center justify-between">
                        <h2 className="text-white font-black text-base">
                            {editId ? '✏️ Tahrirlash' : '➕ Yangi quiz'}
                        </h2>
                        {editId && (
                            <button onClick={cancelEdit} className="text-white/40 hover:text-white text-xs font-bold transition-colors">
                                Bekor qilish ✕
                            </button>
                        )}
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setField('game_type', e.target.value)}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white font-semibold outline-none cursor-pointer"
                                >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setField('badge_type', e.target.value)}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white font-semibold outline-none cursor-pointer"
                                >
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-3">
                            <label className="text-white/40 text-xs font-black uppercase tracking-wider">
                                Savollar ({form.questions.length})
                            </label>
                            <button
                                onClick={addQuestion}
                                className="text-xs font-black px-3 py-1.5 rounded-lg transition-all hover:scale-105"
                            ...
                            {form.questions.map((q, i) => (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1">
                                        ...
                                        {form.questions.length > 1 && (
                                            <button
                                                ...
                                                className="text-red-400/60 hover:text-red-400 text-xs font-black transition-colors"
                                            >
                                                O&apos;chirish ✕
                                            </button>
                                        )}
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-green-500/8 border border-green-500/20 rounded-lg px-3 py-2 text-xs text-green-300 placeholder-green-500/40 font-semibold outline-none focus:border-green-500/40"
                                    ...
                                    {q.wrongAnswers.map((w, wi) => (
                                        <input
                                            ...
                                            className="w-full bg-white/4 border border-white/8 rounded-lg px-3 py-2 text-xs text-white/70 placeholder-white/20 font-semibold outline-none focus:border-white/20"
                    ...
                    {saveMsg && (
                        <div
                            className="px-4 py-2.5 rounded-xl text-sm font-bold text-center"
